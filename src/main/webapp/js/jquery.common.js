...
jQuery.fn.extend({
	countwords:function(errocallbask){
        ...
        if(len > max){
            if(typeof(errocallbask) == "function"){
                errocallbask();  
            }
        } else{
	 ...
	 */
	showMenu : function(selector, callbask) {
	 ...
	 */
	hideMenu : function(selector, callbask) {
		var $selector = $(selector); 
        $selector.attr("show", 0); 
		setTimeout(
			function(){ 
				if($selector.attr("show") == 0){  
					$selector.hide(10, callbask);
				} 
			},
			500
		);
	...
	inputTips : function(){
		...
		box.find(":text,:password,textarea").each(function(i, obj){
			...
			if(typeof(defaultValue) != 'undefined'){
				var nodeId =  $("[id^='sk_prompt_']").size()+1;
				var skLable = '<span class="iptTips" id="sk_prompt_'+nodeId+'">'+defaultValue+'</span>';
				if($(obj).val()==''){
					if($(obj).prev("span.iptTips").length > 0){
						$(obj).prev("span.iptTips").show();
					}else{
						$(skLable).insertBefore($(obj));
					}
					$("#sk_prompt_"+nodeId).unbind().bind("click", function(){
...
jQuery.extend({
	...
	skAjaxBoxy : function(options){
		...
		var boxy = {
			...
			cancel : function(callbask){
				$("#fe_window_mask").remove();
				$("#fe_dialogBox").hide().remove();
				if(typeof(callbask) != 'undefined'){
					callbask();
				}
			}
		};
	...
	skRedirectBoxy : function(options){
		options = jQuery.extend({
			content : '',
			width : false,
			height : false,
	...
	skConfirm : function(options){
		options = jQuery.extend({
			...
			ok : function(){},
			cancel : function(){},
			src:"http://res.skimg.com/sites/www/v2/images/public/ico_warning_24x25.png"
		},options);
		var IE6_scollheight = 0;
        if ( $.browser.msie && $.browser.version == '6.0' ){
                IE6_scollheight = document.documentElement.scrollTop;
		...
		var html = [
				...
				'<img alt="" src="http://res.skimg.com/sites/www/v2/images/public/ico_warning_24x24.png" class="vm mr10">', 
				'<span>'+options.message+'</span>',
				'</p><div class="tc">',
	...
	*/
	skClient : function(options){
		...
		var html = [
			    ...
			    '<div class="fe_dialog" style="width:'+(options.width-14)+'px;height:'+(options.height-14)+'px;">',
				'<div class="loading" style="display:none;"><img src="http://res.skimg.com/sites/www/v2/images/public/dialog/loading.gif" width="" height="" alt="下载中" /></div>',
			    '<div class="contains">',
			    '<div class="hd">'+options.title+'</div>',
			    '<div class="bd">',
	 ...
	 * var loadingBox = $.skTipsBoxy({type:"loading"});
	 * 需调用loadingBox.cancel();关闭
	 */
	skTipsBoxy : function(options){
		options = jQuery.extend({
			message : "",
			type : "success",
			time : 1500,
	...
	skLoginBoxy : function(options){
		...
		var html = [
				...
				'<div class="form_field"><span id="msgCaptip" style="position:absolute;background-color:#FFDBDB;border-radius:4px;color: #1369c0777;padding-left:3px;padding-right:10px;display:none;">大写锁定已打开</span><p class="c_4b9a03 login_tips"></p>',
				...
				'<div style="display:none;" class="fl" id="quisk-login">',
				'<div class="c_quiskLogin">',
				'<h3 class="c_tit">检测到您已经登录KK帐号</h3>',
				'<ul class="c_ul">',
				'<li style="margin-top:20px;"><span class="c_nikename" id="quisk-login-user">我是帐号昵称（10099）</span></li>',
				'<li><a href="javascript:;" id="loginBtn" class="c_btn30_" style="cursor:pointer;text-decoration:none;"><span>快速登录</span></a></li>',
				'<li><a href="javascript:;" id="quit-quisk-login">使用其他帐号登录</a></li>',
				...
				'<dt>使用合作网站帐号登录晒库：</dt><dd><p><img src="http://res.skimg.com/sites/www/v2/images/public/ico_bd_16.png" alt=""> <a class="SinaLoginURL" href="'+KK._baiduLogin+'">用百度帐号登录</a></p><p><img src="http://res.skimg.com/sites/www/v2/images/public/ico_sina_16.png" alt=""> ', 
				'<a class="SinaLoginURL" href="'+KK._sinaLogin+'">用微博帐号登录</a></p><p><img src="http://res.skimg.com/sites/www/v2/images/public/ico_qq_16.png" alt=""> ', 
				'<a class="QQLoginURL" href="'+KK._qqLogin+'">用QQ帐号登录</a></p><p><img src="http://res.skimg.com/sites/www/v2/images/public/ico_rr_16.png" alt=""> ', 
			...
			try{
				...
				if (parseInt(u) > 0) {
					$.getJSON("/?mod=user&act=info&do=prelogin&uid="+u+"&k="+k+"&r="+Math.random(), function(ret){
						if (ret.code == 0) {
							$("#quisk-login-user").html(ret.data.NiskName+"（"+u+"）");
							$("#userLogin_form").hide();
							$("#quisk-login").show();	
						}	
					});
		...
		$("#quit-quisk-login").click(function(){
			$("#userLogin_form").show();
			$("#quisk-login").hide();
			return false;
		});
		...
		$("#loginBtn").click(function(){
			...
			if (parseInt(u) > 0) {
				...
				url = KK.base64_encode(url);
				url = url.replace(/\//, '_');
				location.href = "http://passport.chuanke.com/login/quisklogin?uid="+u+"&k="+k+"&returnurl="+url;
			} else {
		...
		dialogBox.find("a.pop_sign_chuanke").unbind().bind('click', function(){
			dialogMask.remove();
			dialogBox.hide().remove();
			//$.skBoxy({height:520,width:635,content:'<div id="pop_sign"><div class="pop-box"><iframe id="psptframe" src="http://passport.chuanke.com/reg/webpopreg/tm/'+new Date().getTime()+'" frameborder="0" scrolling="No" width="618" height="471"></iframe></div><a href="javascript:;" class="close" id="sign_close" title="关闭弹窗"></a></div>',title:"注册"});
	        //$("#psptframe").attr("src","http://passport.chuanke.com/reg/webpopreg/tm/"+new Date().getTime()+"/");
			var skRegBoxy = $.skAjaxBoxy({
					height:354,
					width:558,
	...
	flashChesk:function(){
		...
		if(isIE){
			try{
				var swf = new ActiveXObject('ShoskwaveFlash.ShoskwaveFlash');
			}catch(e){
				if(isWin8 && isIE){
			...
			if (navigator.plugins && navigator.plugins.length > 0){
				var swf=navigator.plugins["Shoskwave Flash"];
			    if (swf){
					hasFlash=1;
