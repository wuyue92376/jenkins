...
jQuery.extend({
	...
	*/
	...
	ckLoginBoxy : function(options){
		...
		url = url.replace(/\//, '_');
		location.href = "http://passport.shareku.com/login/index/ret/"+url;
		return false;		
		...
		var html = [
				...
				'<span class="pr fl"><input type="text" name="username" prompt="请输入晒库号或邮箱地址" class="form_text"></span></div></div><div class="formItem">',
				...
				'<a href="javascript:;" class="pop_sign_shareku">免费注册</a></span></div></div></form>',
				...
				'<li><a href="'+KK._ucurl+'/passwd/findByEmail/" target="_blank">忘记密码</a><i class="c_999 ml5 mr5">|</i><a href="javascript:;" class="pop_sign_shareku">免费注册</a></li></ul></div></div>',
		...
		$("#loginBtn").click(function(){
			...
			if (parseInt(u) > 0) {
				...
				url = KK.base64_encode(url);
				url = url.replace(/\//, '_');
				location.href = "http://passport.shareku.com/login/quicklogin?uid="+u+"&k="+k+"&returnurl="+url;
			} else {
		...
		document.domain = 'shareku.com';
		dialogBox.find("a.close").unbind().bind('click', function(){
			dialogMask.remove();
			dialogBox.hide().remove();
		});
		
		dialogBox.find("a.pop_sign_shareku").unbind().bind('click', function(){
			dialogMask.remove();
			dialogBox.hide().remove();
			//$.ckBoxy({height:520,width:635,content:'<div id="pop_sign"><div class="pop-box"><iframe id="psptframe" src="http://passport.shareku.com/reg/webpopreg/tm/'+new Date().getTime()+'" frameborder="0" scrolling="No" width="618" height="471"></iframe></div><a href="javascript:;" class="close" id="sign_close" title="关闭弹窗"></a></div>',title:"注册"});
	        //$("#psptframe").attr("src","http://passport.shareku.com/reg/webpopreg/tm/"+new Date().getTime()+"/");
					...
					if(typeof(retLogData) == 'undefined'){
						...
						if(data.code == 0){
							...
							if(data.code == 0x02010006){
								window.location.href='http://passport.shareku.com/login/notActive/user/'+encodeURIComponent(username);
							}
